  ...
  const content = (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid #ECE7DD", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          <p style={{ margin: 0, fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "#7A776F" }}>STERITH POS · BANTUAN</p>
          <p style={{ margin: "2px 0 0", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 16, fontWeight: 700, color: "#0D1117" }}>Kritik & Saran</p>
        </div>
      ...
      {step === "done" ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 24px", textAlign: "center" }}>
          ...
          <p style={{ fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 15, fontWeight: 700, color: "#0D1117", margin: "0 0 6px" }}>Terima kasih!</p>
          <p style={{ fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 12.5, color: "#7A776F", margin: "0 0 24px", lineHeight: 1.6 }}>
            Kritik dan saran Anda sudah kami terima dan akan ditinjau oleh tim Sterith.
          </p>
          <button onClick={handleClose} style={{ background: "#0D1117", color: "#F2EDE3", border: "none", borderRadius: 10, height: 42, padding: "0 28px", fontSize: 12.5, fontWeight: 600, cursor: "pointer", fontFamily: "'Hanken Grotesk', sans-serif" }}>
            Tutup
          </button>
        </div>
      ) : (
        <div style={{ flex: 1, overflowY: "auto", padding: "20px" }}>
          ...
          <p style={{ fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 12.5, color: "#7A776F", lineHeight: 1.6, margin: "0 0 20px" }}>
          ...
          {step === "email" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                <label style={{ display: "block", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 9.5, letterSpacing: "0.18em", textTransform: "uppercase", color: "#7A776F", fontWeight: 600, marginBottom: 7 }}>EMAIL AKUN ANDA</label>
                <div style={{ background: "white", border: "1.5px solid #ECE7DD", borderRadius: 10, padding: "0 13px", height: 46, display: "flex", alignItems: "center", gap: 9 }}>
                  ...
                  <input
                    ...
                    style={{ flex: 1, border: "none", outline: "none", fontSize: 13.5, color: "#0D1117", background: "transparent", fontFamily: "'Hanken Grotesk', sans-serif" }}
                  />
                </div>
                {error && <p style={{ margin: "6px 0 0", fontSize: 11.5, color: "#C25E3D", fontFamily: "'Hanken Grotesk', sans-serif", lineHeight: 1.5 }}>{error}</p>}
              </div>
              <button onClick={verifyEmail} disabled={loading || !email.trim()}
                style={{ height: 46, background: "#0D1117", color: "#F2EDE3", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: loading || !email.trim() ? "not-allowed" : "pointer", opacity: !email.trim() ? 0.5 : 1, fontFamily: "'Hanken Grotesk', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          ...
          {step === "message" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ background: "rgba(92,158,126,0.06)", border: "1px solid rgba(92,158,126,0.2)", borderRadius: 8, padding: "8px 12px", display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 11.5, color: "#5C9E7E", fontFamily: "'Hanken Grotesk', sans-serif", fontWeight: 500 }}>{email}</span>
                <button onClick={() => { setStep("email"); setError(""); }} style={{ marginLeft: "auto", background: "transparent", border: "none", fontSize: 11, color: "#7A776F", cursor: "pointer", fontFamily: "'Hanken Grotesk', sans-serif", textDecoration: "underline" }}>Ganti</button>
              </div>
              <div>
                <label style={{ display: "block", fontFamily: "'Hanken Grotesk', sans-serif", fontSize: 9.5, letterSpacing: "0.18em", textTransform: "uppercase", color: "#7A776F", fontWeight: 600, marginBottom: 7 }}>KRITIK & SARAN ANDA</label>
                <textarea
                  value={message} onChange={e => setMessage(e.target.value)}
                  placeholder="Tulis masukan, saran, atau kendala yang Anda alami…"
                  rows={6}
                  style={{ width: "100%", background: "white", border: `1.5px solid ${message.trim() ? "#C9A55F" : "#ECE7DD"}`, borderRadius: 10, padding: "12px 13px", fontSize: 13.5, color: "#0D1117", fontFamily: "'Hanken Grotesk', sans-serif", resize: "none", outline: "none", lineHeight: 1.6, boxSizing: "border-box" }}
                />
                {error && <p style={{ margin: "4px 0 0", fontSize: 11.5, color: "#C25E3D", fontFamily: "'Hanken Grotesk', sans-serif" }}>{error}</p>}
              </div>
              <button onClick={handleSubmit} disabled={loading || !message.trim()}
                style={{ height: 46, background: "#0D1117", color: "#F2EDE3", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: loading || !message.trim() ? "not-allowed" : "pointer", opacity: !message.trim() ? 0.5 : 1, fontFamily: "'Hanken Grotesk', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
