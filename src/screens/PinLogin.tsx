...
export default function PinLogin() {
  ...
  return (
    <div className="w-full h-full bg-cream-bg flex flex-col animate-screen-in">
      ...
      <div className="flex justify-between items-center px-12 py-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="font-mono text-[9.5px] tracking-eyebrow text-text-mute uppercase mt-1">POINT OF SALE</div>
          </div>
        </div>
        <div className="flex items-center gap-5 text-[13px] text-text-mute">
      ...
      {/* Centered content */}
      <div className="flex-1 flex items-center justify-center px-20 pb-14">
        <div className="w-full max-w-[620px]">
          {/* Store greeting */}
          <div className="text-center mb-10">
            <p className="font-mono text-eyebrow tracking-eyebrow uppercase text-gold mb-3.5">SELAMAT DATANG</p>
            <h1 className="font-serif text-display-xl font-medium text-navy mb-2">Toko Sembako Maju</h1>
            <p className="text-[13.5px] text-text-mute">Jl. Diponegoro No. 24, Palu Timur · Shift Siang</p>
          </div>

          {/* Cashier picker */}
          <p className="font-mono text-[10px] tracking-[0.18em] uppercase text-text-mute mb-3">PILIH KASIR · SELECT CASHIER</p>
          <div className="flex gap-2.5 mb-9">
            {CASHIERS.map(c => {
              const active = selectedCashier === c.id;
              return (
                <button key={c.id} onClick={() => selectCashier(c.id)}
                  className={`flex-1 flex items-center gap-3 rounded-card px-4 py-3.5 relative text-left border transition-colors ${active ? "border-navy" : "border-warm-border"} bg-white`}>
          ...
          {/* PIN dots */}
          <p className="font-mono text-[10px] tracking-[0.18em] uppercase text-text-mute mb-3.5">MASUKKAN PIN · ENTER PIN</p>
          <div className="flex gap-3.5 justify-center mb-9">
            {Array(6).fill(0).map((_, i) => {
              const filled = i < pin.length;
              const active = i === pin.length && pin.length < 6;
              return (
                <div key={i} className={`w-[62px] h-[62px] rounded-card bg-white flex items-center justify-center border ${active ? "border-gold border-[1.5px] shadow-pin-glow" : "border-warm-border"}`}>
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {["1","2","3","4","5","6","7","8","9"].map(d => (
              <button key={d} onClick={() => addPin(d)}
                className="bg-white border border-warm-border rounded-card py-[18px] text-[22px] font-medium text-navy hover:bg-cream-pill transition-colors">
                {d}
              </button>
            ))}
            <button onClick={clearPin}
              className="font-mono text-[12px] text-text-mute rounded-card py-[18px] bg-transparent border-0 hover:text-navy transition-colors">
              LUPA?
            </button>
            ...
            <button onClick={() => pin.length >= 1 && setScreen("sales")}
              className="bg-navy rounded-card py-[18px] flex items-center justify-center gap-2 text-cream-text text-[13px] font-medium tracking-mono-tight hover:bg-navy-soft transition-colors">
              <span>MASUK</span>
