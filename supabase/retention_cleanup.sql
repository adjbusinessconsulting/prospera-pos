...
--   Free 1 day · Standard 30 days · Premium 90 days · Business 1095 · Enterprise 1825
--
...
-- ── STEP 1: retention window per tier (days) ───────────────────────────────────
...
begin
  ...
  where si.sale_id = s.id
    and s.created_at < now() - (public.retention_days(st.tier) || ' days')::interval;
...
-- from public.sales s
-- join public.stores st on st.id = s.store_id
-- where s.created_at < now() - (public.retention_days(st.tier) || ' days')::interval
-- group by st.name, st.tier
-- order by sales_to_delete desc;

-- ── STEP 4: schedule nightly (00:00 WITA = 16:00 UTC). Uncomment to ENABLE. ─────
-- create extension if not exists pg_cron;
-- select cron.unschedule('expire-old-sales')
--   where exists (select 1 from cron.job where jobname = 'expire-old-sales');
-- select cron.schedule('expire-old-sales', '0 16 * * *', $$ select public.expire_old_sales(); $$);
